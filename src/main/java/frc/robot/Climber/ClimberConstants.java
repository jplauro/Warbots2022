package frc.robot.Climber;

public class ClimberConstants {
    public static final int LEFT_WINCH_MOTOR_ID = 7;
    public static final int RIGHT_WINCH_MOTOR_ID = 8;
    public static final int ARMS_SOLENOID_ID = 1;
    public static final int HOOKS_SOLENOID_ID = 2;
    public static final int PROXIMITY_SENSOR_ID = 8;

    public static final double WINCH_LIMIT_MIN = 0;
    public static final double WINCH_LIMIT_MAX = 100;
    public static final double PISTON_FRAMES_MAX = 15;
    
    public static final boolean BRAKE_WHEN_IDLE = true;
}
